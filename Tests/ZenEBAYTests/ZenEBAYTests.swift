import XCTest
@testable import ZenEBAY

final class ZenEBAYTests: XCTestCase {
    
    let accessToken = "v^1.1#i^1#r^0#f^0#I^3#p^3#t^"
    
    lazy var config = Config(endpoint: .sandbox, marketplaceId: "EBAY_IT", locale: "it-IT", accessToken: self.accessToken)
    
    fileprivate func sendFeed(_ requestFeed: RequestFeed) {
        let expectation = XCTestExpectation(description: "eBay API")
        let service = ebayService(config: config)
        service.send(feed: requestFeed) { result -> (Void) in
            expectation.fulfill()
            XCTAssertTrue(result?.messagesWithError == 0)
        }
        wait(for: [expectation], timeout: 10.0)
    }
    
    func testSingleItem() {
        var availability = Availability()
        availability.shipToLocationAvailability = ShipToLocationAvailability(quantity: 1)
        var product = Product()
        product.brand = "Grisolini"
        product.title = "Gaia"
        product.subtitle = "Gaia (Unique, Neutral)"
        product.description = "Single product no variants for test."
        product.ean = ["6889865440152"]
        product.aspects = [
            "Color": ["Neutral"],
            "Size": ["Unique"]
        ]
        product.imageUrls = [
            "http://www.webretail.cloud/media/hwe9-8bZyaEga54mxDXnMg.jpg"
        ]
        let feed = ProductFeed(availability: availability, condition: .new, product: product)
        let requestFeed = RequestFeed(sku: "GAIA12322", feed: feed)
        
        sendFeed(requestFeed)
    }
    
    func testGroupItem() {
        var feed = GroupFeed()
        feed.aspects = ["Wool"]
        feed.title = "Gerardo"
        feed.description = "Grouped product with variants for test."
        feed.imageUrls = [
            "http://www.webretail.cloud/media/2gITOx2cNUxYfFdMua1bfw.jpg",
            "http://www.webretail.cloud/media/hwe9-8bZyaEga54mxDXnMg.jpg"
        ]
        feed.variantSKUs = [
            "WEB1002244-1",
            "WEB1002244-2",
            "WEB1002244-3"
        ]
        let aspectsImageVariesBy = ["Color", "Size"]
        let specifications = [
            Specifications(name: "Color", values: ["White"]),
            Specifications(name: "Size", values: ["Small", "Medium", "Large"]),
        ]
        feed.variesBy = VariesBy(aspectsImageVariesBy: aspectsImageVariesBy, specifications: specifications)
        let requestFeed = RequestFeed(sku: "WEB1002244", feed: feed)
        
        sendFeed(requestFeed)
    }
    
    func testOffer() {
        var feed = OfferFeed()
        feed.sku = "GAIA12322"
        feed.marketplaceId = config.marketplaceId
        feed.availableQuantity = 10
        feed.categoryId = 30120
        feed.listingDescription = "GAIA: Single product no variants for test."
        feed.listingPolicies = ListingPolicies()
        let price = Price(currency: "EUR", value: 100.0)
        feed.pricingSummary = PricingSummary(price: price)
        feed.quantityLimitPerBuyer = 2
        let requestFeed = RequestFeed(sku: "GAIA12322", feed: feed)
        
        sendFeed(requestFeed)
    }
    
    func testPublish() {
        let feed = PublishFeed(offerId: "1023")
        let requestFeed = RequestFeed(sku: "GAIA12322", feed: feed)
        
        sendFeed(requestFeed)
    }
    
    func testPublishGroup() {
        let feed = PublishGroupFeed(inventoryItemGroupKey: "GAIA12322", marketplaceId: config.marketplaceId)
        let requestFeed = RequestFeed(sku: "GAIA12322", feed: feed)
        
        sendFeed(requestFeed)
    }
    
    static var allTests = [
        ("testSingleItem", testSingleItem),
        ("testGroupItem", testGroupItem),
        ("testOffer", testOffer),
        ("testPublish", testPublish),
        ("testPublishGroup", testPublishGroup),
    ]
}
